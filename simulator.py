...
def simulate_haplotypes(frag_file, vcf_file, read_length, coverage, ref_length=int(2.5e7), miscall_rate=0.02, missing_rate=0, per_frag_sw_err=0, snp_rate=0.005, read_length_by_variants=False, span_by_variants=False, coverage_by_variants=False, hic_span = 0, ref_name='chr20'):
    ...
    all_frags.sort(key= lambda x: x[1][0][0])
    with open(frag_file, 'w') as ff:
        for e in all_frags:
            print_fragment(e[0], e[1], ff)
